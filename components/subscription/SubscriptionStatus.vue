    ...
    <CardContent class="space-y-4">
      <div v-if="subscription" class="space-y-4">
        ...
        <div v-if="subscription.subscription" class="space-y-2">
          ...
          <div v-if="subscription.subscription.cancel_at_period_end" class="flex justify-between items-center">
          ...
          <div class="space-y-2">
            <div class="flex justify-between items-center text-sm">
              ...
              <span>
                ...
                <span v-if="subscription.plan.features.max_quizzes !== -1" class="text-muted-foreground">
                  / {{ subscription.plan.features.max_quizzes }}
                </span>
              </span>
            </div>
            
            <div v-if="subscription.plan.features.max_quizzes !== -1" class="w-full bg-gray-200 rounded-full h-2">
              <div 
            ...
            <div class="flex justify-between items-center text-sm">
              ...
              <span>
                ...
                <span v-if="subscription.plan.features.max_participants !== -1" class="text-muted-foreground">
                  / {{ subscription.plan.features.max_participants }}
                </span>
              </span>
            </div>
            
            <div v-if="subscription.plan.features.max_participants !== -1" class="w-full bg-gray-200 rounded-full h-2">
              <div 
        ...
        <div v-if="subscription.payment_method" class="space-y-2">
          ...
          <div class="flex items-center space-x-2 text-sm">
            ...
            <span>{{ subscription.payment_method.brand.toUpperCase() }} •••• {{ subscription.payment_method.last4 }}</span>
...
const handleSync = async () => {
  try {
    loading.value = true;
    const success = await useSubscription.syncSubscription();
    
    if (success) {
      toast({
        title: 'Synchronisation réussie',
        description: 'Les données d\'abonnement ont été mises à jour'
      });
    }
  } catch (error) {
    toast({
