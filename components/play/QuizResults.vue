<template>
  <div class="flex flex-col w-full">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
      <div class="md:col-span-2 bg-white rounded-lg md:rounded-xl shadow-sm p-4 md:p-6 border border-gray-100 w-full">
        <div class="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
          <div class="w-16 h-16 md:w-20 md:h-20 rounded-full flex-shrink-0 flex items-center justify-center"
            :class="isPassed ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'">
            <svg v-if="isPassed" xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 md:h-12 md:w-12" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 md:h-12 md:w-12" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="flex-1 w-full">
            <h3 class="text-xl md:text-2xl font-bold mb-1 md:mb-2" :class="isPassed ? 'text-green-700' : 'text-red-700'">
              {{ isPassed ? "Bravo ! Quiz réussi 🎉" : "Quiz non réussi 😢" }}
            </h3>
            <p class="text-gray-600 mb-2 md:mb-3 text-sm md:text-base">
              {{
                isPassed
            ...
            <div class="flex items-center gap-2 md:gap-4">
              <div class="flex items-baseline gap-1">
                <span class="text-2xl md:text-3xl font-bold" :class="isPassed ? 'text-green-600' : 'text-red-600'">{{ result?.score ?? score }}</span>
                <span class="text-base md:text-xl text-gray-400">/</span>
                <span class="text-base md:text-xl text-gray-700">{{ result?.total ?? totalQuestions }}</span>
              </div>
            </div>
            <div v-if="result?.percentage !== undefined" class="mt-1 md:mt-2 text-gray-500 text-sm md:text-base">
              Pourcentage : <span class="font-semibold">{{ Math.round(result.percentage) }}%</span>
            </div>
            <div v-if="result?.points" class="mt-1 md:mt-2">
              <div class="text-gray-700 font-semibold text-sm md:text-base">Points totaux : <span class="text-pink-600">{{
                result.points.total_points }}</span></div>
              <ul class="text-xs md:text-sm text-gray-600 mt-1">
  ...
  <div class="bg-white rounded-lg md:rounded-xl shadow-sm p-4 md:p-6 border border-gray-100 w-full">
  <h3 class="text-base md:text-lg font-bold mb-3 md:mb-4 text-gray-800 border-b pb-1 md:pb-2">Statistiques</h3>
