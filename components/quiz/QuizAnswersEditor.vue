<template>
  <template v-if="useQuiz.state.quiz?.questions && useQuiz.state.quiz?.questions.length < 1">
    <Label class="text-lg font-bold">Aucune question n'a été créée pour ce quiz.</Label>
    <Button
  ...
  <template v-else>
    <ul class="space-y-4" v-if="useQuiz.state.quiz?.questions">
      <li class="space-y-2" v-for="(question, qIndex) in useQuiz.state.quiz.questions" :key="question.id">
        <div class="flex items-center gap-2">
          <Label> {{ question.content }} ?</Label>
          <Button
            class="bg-pink-100 text-pink-600 font-bold px-3 py-1 rounded-lg hover:bg-pink-200 transition text-sm shadow-sm"
            type="button" @click="create(question.id)">
            + Ajouter une réponse
          </Button>
        </div>
        <ul class="space-y-2">
          <li v-for="(answer, aIndex) in question.answers" :key="answer.id || aIndex" class="flex items-center gap-2">
            <Input v-model="answer.content" />
            <input type="checkbox" :checked="answer.is_correct"
              @change="e => (e.target && (e.target as HTMLInputElement).checked) && setCorrectAnswer(question, aIndex)"
              class="accent-pink-500 w-5 h-5 border-pink-400" />
            <Icon @click="remove(answer.id)" name="Trash" :stroke-width="2" class="text-red-500 cursor-pointer" />
          </li>
        </ul>
      </li>
    </ul>
    <div class="flex items-center gap-2">
      <Button class="bg-pink-500 w-fit text-white font-bold p-2 mt-4 rounded-xl hover:bg-pink-600 transition"
