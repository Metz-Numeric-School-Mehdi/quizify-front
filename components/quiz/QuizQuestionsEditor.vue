<template>
  ...
  <template v-else>
    <ul class="space-y-4">
      <li class="space-y-2" v-for="(question, index) in useQuiz.state.quiz?.questions" :key="question.id">
        <Label> Question {{ index + 1 }} : </Label>
        <div class="flex items-center gap-2">
          <Input v-model="question.content" />
          <Icon @click="remove(question.id)" name="Trash" :stroke-width="2" class="text-red-500 cursor-pointer" />
        </div>
      </li>
    </ul>
    <div class="flex items-center gap-2">
...
const questionsForm = ref<CreateQuestion>({
  ...
  content: useQuiz.state.quiz?.questions && useQuiz.state.quiz.questions.length > 0
