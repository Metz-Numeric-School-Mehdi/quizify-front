<template>
  <div class="flex flex-col gap-4 relative">
    <!-- Fond décoratif comme sur home.vue -->
    ...
    <div class="flex justify-between items-center mb-4 z-10">
      ...
      <DefaultButton :ctaButton="true" @click="openCreateQuizModal">
    ...
    <div v-if="useQuiz.state.quizzes?.length === 0" class="text-center text-gray-500 mt-10">
      <Icon name="Notebook" :size="40" class="mb-2" />
    ...
    <div v-else class="overflow-x-auto">
      <table class="min-w-full bg-white rounded-xl shadow divide-y divide-gray-200">
        ...
        <tbody>
          <tr v-for="quiz in useQuiz.state.quizzes" :key="quiz.id" class="hover:bg-pink-50 transition">
            <td class="px-4 py-2">
              <img v-if="quiz.thumbnail_url" :src="quiz.thumbnail_url" alt="Miniature" class="w-14 h-14 object-cover rounded-lg border" />
              <div v-else class="w-14 h-14 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400">
                <Icon name="Image" :size="24" />
              </div>
            </td>
            <td class="px-4 py-2 font-semibold text-pink-700">{{ quiz.title }}</td>
            <td class="px-4 py-2 text-gray-600">{{ quiz.description?.slice(0, 60) }}<span v-if="quiz.description && quiz.description.length > 60">…</span></td>
    ...
    <CreateQuizModal v-if="showCreateModal" @close="showCreateModal = false" />
  </div>
</template>
